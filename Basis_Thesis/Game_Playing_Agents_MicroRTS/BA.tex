...
\subsection{Results}
The Agent learned to keep the Base defended, even while attacking with other units, which is a substantial improvement over the Base Agent, which often left the Base undefended while attacking.
...
As seen in Tables \ref{tab:base_agent_evaluation} and \ref{tab:main_agent_evaluation}, LT improved the Agent’s performance against (???)all the diverse opponents in the evaluation.
...
The trained Main Agent also performs well against the Base Agent, which it trained against, with a win rate of (???)83\%.
The win rate against RandomBiasedAI also improved from 79\% to (???)92\%, showing that the trained Agent performs substantially better against bots that try to draw the game instead of winning in comparison to the Base Agent.
The agents in the section "Other Bots" are substantially stronger than the built-in bots, which is reflected in the lower win rates against them, but the Main Agent still shows improvements over the Base Agent against (???)all of them.
Highlights include the win rate against Rojo, which improved from 81\% to (???)99\% and the win rate against MixedBot also increased from 43\% to (???)53\%.
While the win rate against Izanagi fell from 72\% to (???)63\%, the loss rate improved from 16\% to (???)11\%, suggesting that the Main Agent now defends the Base more while attacking.
% TODO: entfernen:
