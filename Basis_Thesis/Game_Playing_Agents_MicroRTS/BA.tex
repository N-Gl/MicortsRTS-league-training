...
To address the problem of insufficient diversity we created new initial agents that are focused on different unit types, which can be used as the initial Main Exploiters in the League Training.
This makes it possible for the Agent th exploit weaknesses related to different unit types, without having to unlearn the strategy for the unit type favored by the Base Agent first, which can lead to more diverse exploiters in less training steps.
We only created new initial agents for the Main Exploiters, because we want to focus on training the already strong Base Agent further.
The new initial Agents were not trained long enough, to compare against the Base Agent, but they are strong enough to identify weaknesses in the Main Agent and thereby improve generalization.
