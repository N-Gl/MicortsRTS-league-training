...
If we implement it in this way, a problem arises: player 1 has priority in conflict resolution (e.g. when both players want to move to the same cell).
...
For example, the agent could learn to block an opponent in front of the Base by issuing a simultaneous move to the same target cell.
...
In LT, this is not quite as problematic because there are also exploiters that can be trained against, which remain adversarial to the Main Agent.
...
The Problem is particularly relevant for LT, which combines SP and FSP, because in only historical Agents are available for training in FSP, making the feedback for player 2 very sparse, while in LT the Main Agent can directly train against the current Main Agent.
