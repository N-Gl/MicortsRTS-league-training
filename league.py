        ...
        if (num_done_selfplaygames < 100 or last_logged_selfplay_games + 10 <= num_done_selfplaygames) and ((args.log_exploiter_tables) or isinstance(done_agent, MainPlayer)):
            ...
            if (args.log_exploiter_winrates or isinstance(done_agent, MainPlayer)) and should_log_per_opponent_every_10_games:
                ...
                if isinstance(done_agent, (MainExploiter, LeagueExploiter)):
                    writer.add_scalar(f"{done_agent.name}_charts/agent_steps", agent_steps, args.global_step)
            ...
            writer.add_scalar("charts/main_player_historicals", main_hist, args.global_step)
            writer.add_scalar("charts/main_exploiter_historicals", main_exploiter_hist, args.global_step)
            writer.add_scalar("charts/league_exploiter_historicals", league_exploiter_hist, args.global_step)
            writer.add_scalar("charts/total_historicals", len(historicals), args.global_step)
    ...
    def handle_game_end(self, args, agent, writer, active_league_agents, infos, attack_weight, learning_done_idx, non_learning_done_idx, done_agent, dyn_winloss, hist_reward, num_done_selfplaygames, indices_per_exploiter, learning_indices, last_logged_selfplay_games):
        ...
        if done_agent.ready_to_checkpoint():
            ...
            if isinstance(done_agent, MainExploiter) or isinstance(done_agent, LeagueExploiter):
                ...
                writer.add_scalar(f"{done_agent.name}/num_resets_checkpoints", done_agent.num_resets_checkpoints, args.global_step)
...
def log_bot_game_results(args, writer, infos, attack_weight, done_idx, dyn_winloss, num_done_botgames, done_agent=None):
    ...
    print(
        f"global_step={args.global_step}, episode_reward={episode_reward:.3f}, score_reward_sum={weighted_score_reward_sum:.3f}, bot_winrate_{len(writer.recent_bot_winloss)}={bot_winrate:.3f}, bot_winrate_with_draw_0.5_{len(writer.recent_bot_winloss)}={with_draw:.3f}"
    )
...
def log_exploiter_ppo_update(
    ...
    if (args.exploiter_kle_stop or args.exploiter_kle_rollback):
        if pg_stop_iter == -1:
            writer.add_scalar(f"{player.name}_charts/pg_stop_iter", args.exploiter_update_epochs, args.global_step)
        elif args.exploiter_kle_rollback:
            writer.add_scalar(f"{player.name}_charts/pg_stop_iter", 0, args.global_step)
        else:
