...
class MicroRTSGridModeVecEnv(MicroRTSInterface):
    ...
    def __init__(
        ...
        self.start_client()
        self.num_planes = [1, 2, 3, 8, 6,2]
        self.observation_space = gym.spaces.Box(low=0.0,
                                                high=1.0,
                                                shape=(self._heights[0], self._widths[0],
                                                       sum(self.num_planes)),
                                                dtype=np.int32)
    ...
    def step_wait(self):
        ...
        attackrew = reward[:,4] * self.reward_weight[4]
        reward = np.delete(reward, 0, axis=1)
        reward = np.delete(reward, 7, axis=1)
        reward = np.delete(reward, 4, axis=1)

        weight = np.delete(self.reward_weight, 4)
        weight = weight[1:-1]

        return obs, reward @ weight,attackrew,rewardwinloss, rewardscore, done[:, 0], infos,self._resources,
