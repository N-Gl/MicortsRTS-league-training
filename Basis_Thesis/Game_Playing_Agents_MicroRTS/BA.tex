...
The most notable difference is that, in the training without the new initial agents, no opponents similar to WorkerRushAI developed, resulting in a very low win rate against WorkerRushAI, while the new Main Agent as well as the Base Agent have no difficulty defeating WorkerRushAI.
...
It is likely, that exploiters can realistically find all different strategies within the stratedies for its own focused unit type, however this is a major weakness of the training approach of this thesis: 
exploiters remain close to their initial agents, because otherwise training would take too long. 
As a result, it is possible that not all strategies or weaknesses of the Main Agent are theoretically covered by the exploiters.
However, in the final training, the final unit composition in Figure~(???) per potential initial agent shows that, although rarely, strategies for other unit types are still found. 
This indicates that such strategies are possible, but very time-consuming to learn, and suggests that the diversity of the exploiters is nevertheless relatively high.
For example, the agent focused on Light units learned to also frequently uses Ranged units, even though it was not initialy trained with them. 
This also shows that mixed strategies can be achieved with this training setup.
