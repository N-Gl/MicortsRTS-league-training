...
\break \break
\textbf{Implementation.} To implement SP in the MicroRTS-Py reimplementation Huang et al. implemented the environment to output the observations (obs) and rewards for both players, which allows the agent to train against itself by simply using the same policy for both players \cite{huang2021gym}.
...
\break \break
\textbf{Solutions.} FSP/PFSP are used to stabilize the training and reduce cyclical learning.
To address the second problem, the observations for player 2 are adjusted by mirroring them along the diagonal of the map, meaning that in the observations for the agent, player 2 starts at the top left of the map as well. 
...
\begin{center}
    ...
    \captionof{figure}{Observation adjustment for player 2 in self-play.
    To mirror the observations the GridNet representation was flipped for every player-two environment.
...
The same mirroring as for the observations is applied for the invalid action masks and the observations, while actions, including the position component of the action space was adjusted after the masking.
...
\break \break
\textbf{Implementation.} The implementation is similar to SP, but instead of Player 2 always using the current policy, Player 2 is sampled uniformly from a pool of historical agents (snapshots of the Main Agent at different training stages).
\break \break
\textbf{Problem with the implementation.} If we implement in the described way, a problem arises: player 1 has priority in conflict resolution (e.g. when both players want to move to the same cell).
...
\break \break
\textbf{Example.} The agent could learn to block an opponent in front of the Base by issuing a simultaneous move to the same target cell.
...
\break \break
\textbf{Relevance for LT.} In LT, the Result of the problem is not quite as problematic because there are exploiters that can be trained against, which remain adversarial to the Main Agent.
...
\end{figure}
\break \break
\textbf{Solutions.} In the re-implementation of MicroRTS-Py, there is a mode that resolves conflicts in a randomized manner.
...
% TODO: füge ein Bild ein, in dem die Ergebnisse von 2 gegeneinander spielenden Agenten 1 mal als Spieler 1, 1 mal als Spieler 2 dargestellt werden.
...
\break \break
\textbf{Prioritization Function.} The exact prioritization function can differ for different agent types.
...
\break \break
\textbf{Implemented Prioritization Function.} To avoid opponents resulting in little learning signals, the following prioritization function was used for the Main Exploiters:
