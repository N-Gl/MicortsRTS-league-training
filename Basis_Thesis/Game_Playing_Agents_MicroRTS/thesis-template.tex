...
Die Built-in-Bots von MicroRTS, gegen die unteranderen ausgewertet wird sind RandomBiasedAI, WorkerRushAI, LightRushAI, CoacAI.
\begin{table}[ht]
\centering
...
Zu openai/baselines wurde unteranderem \emph{invalid action masking} und geeignete Aktions-/Beobachtungsrepräsentationen (z.\,B.: GridNet \cite{pmlr-v97-han19a}), sowie architektonische Entscheidungen (, wie IMPALA-CNN (eine tiefe Convolutional-Neural-Network-Struktur mit residual blocks zur Verarbeitung gitterbasierter Zustandsrepräsentationen in RL) \cite{espeholt2018impala}) hinzugefügt \cite{huang2021gym}, die auch abgeändert in dem LT Code noch benutzt werden.
...
GridNet hingegen sagt für jede Zelle auf der Karte eine Aktion voraus. Das macht GridNet mit einem großen Aktionsraum in einem Schritt, was den Aktionsraum größer, aber simplistischer macht \cite{huang2021gym, pmlr-v97-han19a}.
...
\cite{goodfriend2024raisocketai} RAISocketAI ist der erste DRL-Agent, der die IEEE MicroRTS Competition gewinnen konnte. 
RaiSocketAI beinhaltet eine neu implementation von MicroRTS-Py, die in unserem Code verwendet wird. 
Unteranderem ist die Datengenerierung über Replays ist eine Wichtige Grundlage für BC \cite{huft2023microrts} und es wurde ein Bug in der alten implementation behoben, der nicht besitzte Recourcen als besitzte Recourcen markierte, wenn der Agent Spieler 2 ist. 
Das führte Wahrscheinlich auch dazu, dass SP in der vorherigen Implementation von MicroRTS-Py keine Verbesserungen brachte \cite{goodfriend2024raisocketai}.
Die RAISocketAI-Implementierung von FSP hat gegen die gleichen built-in-Bots von MicroRTS wie in MicroRTS-Py eine durchschnittliche Winrate von 91\% erreicht, was deutlich höher ist als die 84\% in der vorherigen Implementation \cite{goodfriend2024raisocketai}. 
Allerdings erreichte FSP gegen CoacAI nur eine 20\% Winrate, wärend die besten vorherigen Implementationen fast immer CoacAI besiegen konnten \cite{goodfriend2024raisocketai}.
...
LT wurde in der Arbeit noch nicht implementiert oder evaluiert.
\vspace{10pt}

\noindent\textbf{BeClone}
\cite{martin2021beclone} BeClone zeigte die effectivität von BC mit DRL-finetuning in MicroRTS-Py mit einer zweiphasigen Trainingsstrategie für RTS-Agenten: Zuerst wird eine initiale Policy per BC von den Experten gelernt, anschließend erfolgt ein RL-Finetuning mittels \emph{Advantage Actor-Critic} (A2C).
\vspace{10pt}
...
Der Agent wurde mit Imitation Learning aus menschlichen Replays vortrainiert. Das haben wir in dieser Arbeit mit den BC Phasen und der PPO Phase ersetzt. 
...
Gleichzeitig trainieren League-Exploiters, gegen alle Historischen Agenten \cite{vinyals2019alphastar}.
Der Main-Agent trainiert mit prioritized FSP (PFSP) gegen historische Versionen von sich selbst, Main-Exploiters und League-Exploiters \cite{vinyals2019alphastar}.
...
Erst, wenn ein Exploiter eine große Winrate gegenüber allen Gegnern aufweist oder, wenn eine bestimmte Maximale Step Grenze erreicht wird, wird er in die Liga als Historical (ein historischer Agent) aufgenommen \cite{vinyals2019alphastar}. 
In AlphaStar kommt ein Actor-Critic-Verfahren mit einer Off-Policy Korrekturmethode namens V-trace für den Policy-update zum Einsatz \cite{espeholt2018impala, vinyals2019alphastar}. 
...
Dazu wird in AlphaStar Temporal difference learning (TD($\lambda$)) \cite{suttonbarto1998rl} für die Value-Schätzungverwendet.
