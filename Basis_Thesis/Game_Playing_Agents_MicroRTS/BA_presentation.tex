...
\begin{frame}[noframenumbering]{Evidence}
    ...
    \begin{block}{Is the improvement really caused by LT, or just by more training?}
        \begin{itemize}
            ...
            \item The final agent reaches \textbf{85.7\%} after PPO fine-tuning.
            \item A same-compute PPO baseline would be the cleanest additional control.
        \end{itemize}
    \end{block}
...
\begin{frame}[noframenumbering]{Expected Questions: Generalization}
    \small
    \begin{block}{Can we call this generalization with only one map?}
        \begin{itemize}
            ...
            \item Generalization is tested across opponent types, not across maps.
            \item Several improved opponents were not used in LT training, especially MCTS-based bots.
        \end{itemize}
    \end{block}
...
\begin{frame}[noframenumbering]{Expected Questions: PPO and AlphaStar}
    \small
    \begin{block}{Why use PPO instead of AlphaStar's V-trace setup?}
        \begin{itemize}
            \item PPO was already available in the MicroRTS/RAISocketAI training pipeline.
            \item It made LT practical to implement within the scope of this thesis.
    ...
    \begin{block}{What is different from AlphaStar?}
        \begin{itemize}
            \item smaller environment and much smaller league
            \item PPO instead of off-policy actor-critic with V-trace
            \item initial policy from BC/PPO bots, not from broad human replay data
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}[noframenumbering]{Expected Questions: Diversity}
    \small
    \begin{block}{Why is exploiter diversity the main limitation?}
        \begin{itemize}
            \item Exploiters initialized from the same Base Agent tend to find similar weaknesses.
            \item The Base Agent strongly favors heavy-rush behavior.
            \item Different strategy clusters, such as worker or ranged strategies, are hard to discover from that start point.
        \end{itemize}
    \end{block}
    \begin{block}{Why add new initial agents?}
        \begin{itemize}
            \item They give exploiters different strategic starting points.
            \item This improved difficult matchups, e. g. Droplet and GuidedRojoA3N.
            \item It supports the hypothesis that BC expert diversity strongly affects LT performance.
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}[noframenumbering]{Expected Questions: Bot Environments}
    \small
    ...
    \begin{block}{Does this make the evaluation unfair?}
        \begin{itemize}
            ...
            \item The stronger evidence comes from opponents not used for LT training.
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}[noframenumbering]{Expected Questions: Player Side}
    \small
    ...
    \begin{block}{Does Player 1 priority bias the results?}
        \begin{itemize}
            ...
            \item Self-match evaluation: P1 \textbf{38\%}, draw \textbf{28\%}, P2 \textbf{34\%}.
            \item Alternating the learning side prevents purely P1-specific behavior.
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}[noframenumbering]{Expected Questions: Draws and NaiveMCTS}
    \small
    \begin{block}{Why are win rates against NaiveMCTS so low?}
        \begin{itemize}
            \item NaiveMCTS often avoids losing instead of actively trying to win.
            \item This creates many draws and few losses.
            \item Therefore the loss rate is important for interpreting this matchup.
        \end{itemize}
    \end{block}
...
\begin{frame}[noframenumbering]{Expected Questions: Statistical Reliability}
    \small
    \begin{block}{Are the improvements statistically reliable?}
        \begin{itemize}
            ...
            \item Only one LT training seed was used because training was expensive.
        \end{itemize}
    \end{block}
    \begin{block}{What is the cleanest future experiment?}
        \begin{itemize}
            \item same compute budget for PPO, LT and LT + FT
            \item several random seeds
            \item ablations for new initial agents, bot environments, delta score and entropy annealing
        \end{itemize}
    \end{block}
\end{frame}
