...
The results show that the exploiter are able to efficiently exploit the Main Agent, while staying within their respective unit type focus.
This suggests, that exploiter diversity within the set of strategies for their respective focused unit type is sufficient, to find weaknesses in the Base Agent.
...
As a result, it is possible for strategies or weaknesses to exist in the Main Agent, that are only exploitable by similarly difficult to reach exploiter states, that are unlikely to be represented by an historical exploiter after the training.
...
We have initial agents for each unit type for the exploiters, the many examples of working exploits within the set of strategies for the focused unit types and many other examples, that also show the exploiters capability of exploring strategies with mixed unit types. 
This suggests that the exploiter set is diverse enough to cover many relevant strategy variations and improve the performance of the Base Agent, even for other agents not in the training or evaluation.
