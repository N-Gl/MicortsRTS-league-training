...
import microrts_interface
from microrts_interface import (
    ByteArray,
    MicroRTSInterface,
...
class MicroRTSGridModeVecEnv(MicroRTSInterface):
    ...
    def __init__(
        ...
        self.microrts_path = os.path.join(Path(__file__).parent.parent, "microrts")
        ...
        self.action_space = gym.spaces.MultiDiscrete([
            self._heights[0]  * self._widths[0],
            6, 4, 4, 4, 4,
    ...
    def step_wait(self):
        ...
        return np.array(obs), reward @ weight, rewardwinloss, rewardscore, done[:, 0], infos, self.actions
    ...
    def get_game_state(self, env_idx: int):
        if env_idx < self.num_selfplay_envs:
            return self.vec_client.selfPlayClients[env_idx//2].gs
        else:
            return self.vec_client.clients[env_idx - self.num_selfplay_envs].gs
