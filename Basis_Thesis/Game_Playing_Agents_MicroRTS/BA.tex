...
The map used is basesWorkers16x16A, a symetrical map also used in the related work such as \cite{huang2021gym, huft2023microrts, goodfriend2024raisocketai}.
...
The following Table \ref{tab:checkpoint_statistics} shows the amount of checkpoints for each agent type and the win rate of the Agent type against the Main Agent avraged over the last 200 games for each reset.
\begin{table*}[t]
\centering
\caption{checkpoint and reset statistics for the League Training setup}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}l c c r@{}}
\hline
\textbf{Agent type} & \textbf{League Exploiter} & \textbf{Main Exploiter} & \textbf{acg. win rate} \\
\hline
    Base Agent & (???) & (???) & (???) \\
    Worker\_focused & 0 & (???) & (???) \\
    Ranged\_focused & 0 & (???) & (???) \\
