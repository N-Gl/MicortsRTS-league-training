...
LT mitigated the SP problem of cyclic training, however there is still a similar problem with exploiter diversity.
While LT has a finite number of weaknesses that can be exploited within a training run, in complex environments such as MicroRTS, there are many more weaknesses that can be exploited by only slightly different strategies, than there are historical exploiters that can be created during training.
This means that the exploiters will most likely only cover strategies that can be exploited by strategies similar to the exploiters’ initial agents.
We hypothesize that the diversity of the initial agent is crucial for the diversity of the exploiters, and possibly more important than the initial agent's strength, because the initial agents of the exploiters strongly influence the likely to be reached strategy space of the exploiters.
It is likely that this was not as big of a problem in AlphaStar, because the initial agent was trained with imitation learning on human data, which covered a much wider range of strategies, causing the initial agent to be competent across a broader range of strategies than our Base Agent used in this thesis, which was trained with BC and PPO against only two built-in bots and focused on one strategy.
This diversity of the initial agent likely led to many more diverse strategies being similar to the exploiters’ initial agents, which made it more likely that an exploiter would exploit those diverse strategies.
A more diverse set of exploiters would then improve the Main Agent's opponent diversity and generalization.
Further constraining the diversity of our Base Agent is that, in BC and PPO, we even have to leave some possible opponents out of training to obtain a more robust evaluation, and many strong agents are too slow to be trained against.
In contrast, in AlphaStar, the agent was evaluated against human players.
...
% TODO: Alphastar nutzte human imitation learning --> Base Agent ist diverser --> diversere Exploiters --> bessere Main Agent Performance, evaluation gegen Menschen besser als gegen Bots, da Menschen diversesität gegen bots nur funktioniert, indem man nicht gegen ihn trainiert.


\paragraph{Different Maps.}
