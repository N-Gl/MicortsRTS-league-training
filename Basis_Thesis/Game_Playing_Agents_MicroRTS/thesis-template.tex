...
\begin{titlepage}
    \begin{center}
        ...
        {\Huge\textbf{Implementierung und Evaluation der Reinforcement-Learning-Methoden Self-Play und Liga-Training in der Micro-RTS-Umgebung}}\\[2cm]
...
\noindent\textbf{Game Playing Agents MicroRTS}
Die dieser Arbeit zugrunde liegende (unveröffentlichte) Bachelorarbeit, implementierte verschiedene Agenten für Micro-RTS und kombinierte und evaluierte BC und PPO in der Micro-RTS Umgebung \cite{huft2023microrts}. Aus dieser Bachelorarbeit stammt der Basis-Agent und der initiale Code, der in dieser Arbeit weiter verbessert.
Die dabei verwendete hybride Trainingspipeline besteht aus drei aufeinanderfolgenden Trainingsphasen: BC, BC-FT und abschließendem Training mittels PPO. \cite{huft2023microrts}.
\begin{table}[ht]
\centering
\caption{}
...
Huang et al. stellen mit \emph{Gym-$\mu$RTS} bzw. \emph{MicroRTS-Py} eine effiziente OpenAI-Gym-kompatible Schnittstelle für MicroRTS bereit und etablieren damit eine weit verbreitete DRL-Baseline \cite{huang2021gymurts}. In ihrer Arbeit zeigen sie, dass PPO in MicroRTS insbesondere dann konkurrenzfähig trainierbar ist, wenn geeignete Engineering-Maßnahmen eingesetzt werden, darunter \emph{invalid action masking}, geeignete Aktions-/Beobachtungsrepräsentationen (z.\,B. GridNet) sowie architektonische Entscheidungen (u.\,a. IMPALA-CNN) \cite{huang2021gymurts, huang2020invalidmask, huang2019representations}. Diese Arbeiten bilden den methodischen Ausgangspunkt vieler nachfolgender Deep-RL-Ansätze in MicroRTS.

\noindent\textbf{RAISocketAI}
RAISocketAI ist der erste Deep-RL-Agent, der die IEEE MicroRTS Competition gewinnen konnte, und setzt damit einen wichtigen Referenzpunkt für DRL-basierte Ansätze in dieser Domäne \cite{goodfriend2024raisocketai}. Zentral für die Performance sind laut Goodfriend ein iteratives Fine-Tuning der Policy, ein abgestimmtes Trainingsregime sowie Transfer-Learning zwischen Karten-/Problemgrößen \cite{goodfriend2024raisocketai}. Zusätzlich beschreibt die Arbeit Erweiterungen und praktische Verbesserungen an der MicroRTS-Py-Toolchain, die u.\,a. die Datengenerierung über Replays erleichtern und damit auch für Imitation-Learning-Ansätze relevant sind.

\noindent\textbf{BeClone}
BeClone untersucht eine zweiphasige Trainingsstrategie für RTS-Agenten: Zunächst wird eine initiale Policy per BC aus Demonstrationen gelernt, anschließend erfolgt ein RL-Finetuning mittels \emph{Advantage Actor-Critic} (A2C) im Self-Play \cite{martin2021beclone}. Der Fokus der Experimente liegt auf ressourcenorientierten Aufgaben und vergleichsweise kleinen Karten, wodurch BeClone primär als Nachweis dient, dass BC als Warm-Start die anschließende RL-Optimierung in RTS-Settings beschleunigen und stabilisieren kann \cite{martin2021beclone}. Im Gegensatz dazu zielt die vorliegende Arbeit auf die Implementierung und Evaluation von Self-Play und Liga-Training in einem stärker kompetitiven Setup mit PPO als Optimierungsverfahren.
