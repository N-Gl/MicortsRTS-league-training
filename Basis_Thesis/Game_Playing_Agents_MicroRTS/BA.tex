...
With LT the agent even achieved greater win rates against the bots, that were used in the Base Agents training, than the Base Agent itself, which shows that with LTs generalization is an improved state over the generalization achivable, by stopping PPO from overfitting.
In PPO, when an generalized state is reached the training has to be stopped however there are more possible agent states, that are just as generalized and stronger against even the trainings opponents, but the training cannot reach them, because it would overfit.
Meaning with PPO we can only see a tiny subset of the possible generalized states between when the agent improved and when it overfitted, while with LT we can reach many more of these states, which can lead to much stronger agents.
