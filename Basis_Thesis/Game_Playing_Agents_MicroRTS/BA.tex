...
LT removed the SP problem of cyclic training, but there is still a similar problem with exploiter diversity.
While we now have a finite number of weaknesses that can be exploited within an training run, in complex environments such as MicroRTS, there are a lot more weaknesses, that can be exploited by only slightly different strategies, than there are historical exploiters that can be created during training.
That means that the exploiters will most likely only cover strategies exploitable by similar strategies to the exploiters initial agent.
We hypothesize, that diversity of the initial agent is crucial for the diversity of the exploiters, and more possibly important than the initial agent's strength.
It is likely that this was not as big of a problem in AlphaStar, because the initial agent was trained with imitation learning on human data, which covered a much wider range of strategies, causing the inital agent to be strong in more strategies than our Base Agent used in this thesis, which was trained with BC and PPO against only two built-in bots and focused on one strategy.
This diversity of the initial agent likely led to a lot more diverse strategies beeing similar to the exploiters initial agents, wich made it more likely that an exploiter would exploit those diverse strategies.
A more diverse set of exploiters would then improve the Main Agent's opponent diversity and generalization.
Further constraining diversity of our Base Agent is, that in BC and PPO we even have to leave some of the possible opponents out of the training, to have a more robust evaluation and many strong agents are too slow to be trained against.
While in AlphaStar, the agent was evaluated against human players.
