...
The agents in the section "Other Bots" are considerably stronger than the built-in bots, which is reflected in the lower win rates against them, but the Main Agent still shows improvements over the Base Agent against ((???)all) most of these opponents.
...
The results show that the exploiters are able to efficiently exploit the Main Agent while staying within their respective unit type focus.
This suggests that exploiter diversity is sufficient to identify and exploit weaknesses within the set of strategies for their respective focused unit type of the Base Agent.
...
As a result, it is possible for strategies or weaknesses to exist in the Main Agent that are only exploitable by similarly hard-to-reach exploiter states, which are unlikely to be represented by a historical exploiter after training.
We do not know whether such weaknesses actually exist.
...
However, the final unit composition in Figure~(???) for each potential initial agent shows that at least some mixed strategies can be achieved with this training setup.
For example, the agent focused on Light units learned to also frequently use Ranged units, even though it was not initially trained with them.
We have initial agents for each unit type for the exploiters, as well as examples of working exploits within the set of strategies for the focused unit types, and many other examples that also show the exploiters' capability to explore strategies with mixed unit types.
This suggests that the exploiter set is diverse enough to cover many relevant strategy variations and improve the performance of the Main Agent, even against other agents not included in the training or evaluation.

% Wahrscheinlich kann es unterschiedliche Strategie innerhal der eigenen unit types finden,
% das ist aber auch die größte schwachstelle dieses Trainings (Exploiter bleiben in der Nähe ihrer initialen Agtenten, weil sonst das Training zu lange dauert, aber dadurch kann es passieren, das nicht alle Strategien/Schwachstellen des Main Agent abgedeckt werden.
% Allerdings kann man in dem entgültigen Training durch die Anzahl pro unit am ende des Spieles für die fokussierten initialen Agenten erkennen, das zwar selten aber doch Strategien für andere unit types gefunden werden, was zeigt, dass es möglich ist, aber eben sehr zeitaufwendig ist und das die "diversity" der Exploiter dennoch hoch ist.
% Beispielsweise benutzt der Agent der auf Light units fokusiert auch gerne Ranged Units, mit denen er nicht trainiert wurde. (das zeigt, das gemischte Strategien erreicht werden können)
